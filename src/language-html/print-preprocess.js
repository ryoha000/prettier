...
const PREPROCESS_PIPELINE_WALK = [
  removeIgnorableFirstLfOriginal,
  mergeIeConditonalStartEndCommentIntoElementOpeningTagOriginal,
  mergeCdataIntoTextOriginal,
  extractInterpolationOriginal,
  extractWhitespacesOriginal,
  addCssDisplayOriginal,
  addIsSelfClosingOriginal,
  addHasHtmComponentClosingTagOriginal,
  addIsSpaceSensitiveOriginal,
  mergeSimpleElementIntoTextOriginal,
];

function preprocessOriginal(ast, options) {
...
function removeIgnorableFirstLfOriginal(ast /*, options */) {
  return ast.map((node) => {
    if (
      node.type === "element" &&
      node.tagDefinition.ignoreFirstLf &&
...
function mergeIeConditonalStartEndCommentIntoElementOpeningTagOriginal(
  ast /*, options */
) {
  /**
   *     <!--[if ...]><!--><target><!--<![endif]-->
...
function mergeNodeIntoTextOriginal(ast, shouldMerge, getValue) {
  return ast.map((node) => {
    if (node.children) {
      const shouldMergeResults = node.children.map(shouldMerge);
      if (shouldMergeResults.some(Boolean)) {
...
function mergeCdataIntoTextOriginal(ast /*, options */) {
  return mergeNodeIntoTextOriginal(
    ast,
    (node) => node.type === "cdata",
    (node) => `<![CDATA[${node.value}]]>`
  );
...
function mergeSimpleElementIntoTextOriginal(ast /*, options */) {
  const isSimpleElement = (node) =>
    node.type === "element" &&
    node.attrs.length === 0 &&
    node.children.length === 1 &&
...
function extractInterpolationOriginal(ast, options) {
  if (options.parser === "html") {
    return ast;
  }
 ...
 * - remove insensitive whitespaces
 */
const WHITESPACE_NODE_ORIGINAL = { type: "whitespace" };
function extractWhitespacesOriginal(ast /*, options*/) {
  return ast.map((node) => {
    if (!node.children) {
      return node;
    }
    ...
    return node.clone({
      ...
      children: node.children
        ...
        .flatMap((child) => {
          ...
          if (leadingWhitespace) {
            localChildren.push(WHITESPACE_NODE_ORIGINAL);
          }
        ...
        // set hasLeadingSpaces/hasTrailingSpaces
        .map((child, index, children) => {
          if (child === WHITESPACE_NODE_ORIGINAL) {
            return;
          }

          return {
            ...child,
            hasLeadingSpaces: children[index - 1] === WHITESPACE_NODE_ORIGINAL,
            hasTrailingSpaces: children[index + 1] === WHITESPACE_NODE_ORIGINAL,
          };
        })
        // filter whitespace nodes
        .filter(Boolean),
...
function addIsSelfClosingOriginal(ast /*, options */) {
  return ast.map((node) =>
    Object.assign(node, {
      isSelfClosing:
        !node.children ||
...
function addHasHtmComponentClosingTagOriginal(ast, options) {
  return ast.map((node) =>
    node.type !== "element"
      ? node
      : Object.assign(node, {
...
function addCssDisplayOriginal(ast, options) {
  return ast.map((node) =>
    Object.assign(node, { cssDisplay: getNodeCssStyleDisplay(node, options) })
  );
}
 ...
 * - add `isDanglingSpaceSensitive` field for parent nodes
 */
function addIsSpaceSensitiveOriginal(ast, options) {
  return ast.map((node) => {
    if (!node.children) {
      return node;
    }
